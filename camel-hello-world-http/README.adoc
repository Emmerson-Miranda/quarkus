= Observability: HTTP Hello World example
...
[source,shell]
----
% curl -H "x-correlation-id: myIdGet" http://localhost:8080/greeting
% curl -X POST  -d 'This is request payload' -H "x-correlation-id: myIdPost" http://localhost:8080/greeting
% curl -H "x-correlation-id: myIdGetGoogle" http://localhost:8080/getgoogle
----
...
$ curl -s -H"Accept: application/json" localhost:8080/q/metrics/application \
    | jq '.["camel.context.exchanges.completed.total;camelContext=camel-hello-world-http"]'
----
